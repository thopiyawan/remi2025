...
private function analyzeImageWithGemini($imageBinary)
{
    ...
    [
        ...
        'parts' => [
            ...
            'text' => '
                  ...
                  วิเคราะห์ภาพอาหารและตอบกลับเป็น JSON เท่านั้น analysis และ recommendations ห้ามตอบยาวเกิน 500 ตัวอักษร ขอสั้นกระชับเข้าใจง่าย ขอเป็นหน่วยทัพพี ช้อนโต๊ะ ชิ้น ที่คนอ่านเข้าใจง่ายๆ
                  ห้ามใช้ markdown
                  ห้ามใส่ ```json
                  ประเมิน:
                  - อาหาร
    ...
    if (isset($data['candidates'][0]['content']['parts'][0]['text'])) {
        ...
        $meal = json_decode($resultJson, true);

        $flex = $this->buildMealFlex(
            $meal,
            $todayCalories,
            $targetCalories
        );
...
public function chatWithGemini($userMessage)
{
    ...
    $payload = [
        ...
        'systemInstruction' => [
            'parts' => [
                ['text' => 'คุณคือผู้ช่วยหญิงตั้งครรภ์อัจฉริยะในระบบ REMI ตอบคำถามต่างๆอย่างกระชับ สั้นๆ' .
                   'ข้อกำหนดสำคัญ: ห้ามใช้เครื่องหมาย Markdown เช่น ** หรือ * ใดๆ ทั้งสิ้น' .
                   'ห้ามตอบยาวเกิน 800 ตัวอักษร']
            ]
        ],
        'generationConfig' => [
...
private function buildMealFlex(array $meal, int $todayCalories = 0, int $targetCalories = 2100)
{
    $analysisText = "• " . implode("\n• ", $meal['analysis']['message']);

    $recommendText = "• " . implode("\n• ", $meal['recommendations']);
    ...
    return [
        ...
        "header" => [
            ...
            "contents" => [
                ...
                [
                    "type" => "text",
                    "text" => "🟡 มื้อนี้ : " . $this->riskLabel($meal['analysis']['risk_level']),
                    "color" => "#FFEAA7",
                    "size" => "sm",
                    "weight" => "bold",
                    "margin" => "xs"
            ...
            "contents" => [
                ...
                [
                    ...
                    "contents" => [
                        ...
                        [
                            "type" => "text",
                            "text" => "{$todayCalories} / {$targetCalories} kcal",
                            "align" => "end",
                            "size" => "sm",
                            "weight" => "bold",
                            "color" => "#27AE60"
...
private function riskLabel($risk)
{
    return match ($risk) {
        'low' => 'ดี',
        'medium' => 'พอใช้',
